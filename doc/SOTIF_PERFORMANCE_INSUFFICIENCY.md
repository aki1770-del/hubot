...
velocity in the translation unit (`src/zone_parameter_filter.cpp:525-528`). The window this
...
| **PI-2** | **The declared expiry exceeds the publisher's own threshold for calling itself stale, and neither number has a ceiling.** | Shipped defaults. No misconfiguration required. | `valid_for_s: 2.5` while `costmap_silence_timeout` is `2.0`. | `README.md:342` instructs: *"If `now - header.stamp > valid_for_s`, stop believing the message."* Read literally — and it is written to be read literally, *"by three lines of consumer code"* — the consumer keeps believing an `enforced: yes` for **2.5 s** after the last message, while the publisher would have disowned its own reading at **2.0 s**. Widens without bound: `liveness_period: 30.0` → `valid_for_s: 75.0`, and on a moving platform that is a distance, not a time. | `cpp:59` (`kValidForPeriods = 2.5`) · `cpp:1009-1016` (`declaredValidForS()`, where the product and the clamp now live) · `hpp:552` (`costmap_silence_timeout_{2.0}`) · `README.md:342` · **no clamp on either parameter anywhere in the translation unit** | ⚑ **OPEN — CONFIRMED BY EXECUTION 2026-09-06 (SC-3 RED)** |
...
| **PI-4** | ⚑ **The only complete countermeasure the component recommends is structurally incompatible with the component's own publisher.** | An integrator follows `README.md:561` and requests a finite `DEADLINE` QoS on their `zone_decision` subscription. | Under DDS Request/Offered matching, an **offered** deadline of infinity does not satisfy any **finite** requested deadline. The subscription does not match; the consumer receives **nothing at all** — which they are most likely to read as a broken topic, not as *"not watching."* | `decision_pub_` is was created with a bare `rclcpp::QoS(10)` (the record of that is at `cpp:202`; it now carries a deadline at `cpp:228-233`), leaving the offered deadline at the rmw default (infinite). The package tells the integrator to build the one mechanism it prevents them from using. **One-line fix on the publisher side:** offer a deadline derived from `liveness_period`. | `cpp:202` · `cpp:228-233` · `README.md:556-565` · `hpp:633-636` | ⚑ **OPEN — CONFIRMED BY EXECUTION 2026-09-06 (SC-5 RED; rmw: "No messages will be sent to it… DEADLINE_QOS_POLICY")** |
...
> ### ⚑ TWO STATUS CELLS ABOVE ARE CONTRADICTED BY OTHER DOCUMENTS ON THIS SAME TREE.
>
> **Raised 2026-09-06 by CPP while repairing this document's line citations. CPP does not
> hold the pen on a SOTIF verdict and has not moved either cell — this is a finding, routed,
> not a ruling.** Recorded here rather than in a report nobody opens, because the cells are
> read as current.
>
> - **PI-2 says `SC-3 RED`.** `CHANGELOG.md:86` says the defect is fixed and *"SC-3 stays as
>   written and is green on both substrates."* ⚑ **And `CHANGELOG.md:94`, in the same file,
>   lists SC-3 among the three tests that do NOT pass against release `1.5.1`.** Those two
>   CHANGELOG sentences cannot both be true, independently of this table.
> - **PI-4 says `SC-5 RED`** and cites a publisher *"created with a bare `rclcpp::QoS(10)`"*.
>   Measured on this tree: `src/zone_parameter_filter.cpp:228-233` builds `decision_qos` and
>   sets `.deadline(declaredValidForS())` whenever `liveness_period > 0`; the bare-QoS state
>   survives only as the record of what it was, at `cpp:202`. `CHANGELOG.md:100` reports
>   *"SC-5 green, both arms."* **The evidence column described code that is no longer here,
>   and the citation is repaired; the VERDICT is untouched.**
>
> **Neither can be settled from this host: `colcon` cannot run here, so the oracles were not
> executed this turn and no green or red is claimed by this note.** For FSE (verdict author)
> and AAA (safety-class audit) to re-run and reconcile. Until then the three documents
> disagree, and a reader should trust none of the three cells over the others.
...
> README and the CHANGELOG and did not reach the header.** `hpp:528` still reads *"a PROMISE with
...
> `src/zone_parameter_filter.cpp:373` applies nav2's `joinWithParentNamespace()` to your `node:`
...
who follows `README.md:561` gets **silence in their own process**, while the diagnostic explaining
...
> as a negative control and refused. The citation at `cpp:479` was a bare number with no text (repaired 2026-09-06 to `cpp:585`, the line that names the vision) — it
